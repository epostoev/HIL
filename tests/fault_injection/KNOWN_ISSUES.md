...
7. **[ИСПРАВЛЕНО 2026-08-04] `base_hil_test.py:255-260` — `get_pid()` без фильтра `grep -v grep -v python3`.**
   ...
   **Уточнение (после разбора `Makefile`, см. `tests/HOW_IT_WORKS.md`):**
   ...
   Исправлено: восстановлен фильтр `| grep -v grep | grep -v python3`. Код
   фикса не зависит от того, какая из двух причин точнее объясняет риск —
   фильтр закрывает обе. Остаточный риск (не устранён этим фиксом): если
   после фильтрации всё равно совпадёт несколько строк (например, несколько
   worker-процессов одной ноды), `[0]` по-прежнему выберет первую в порядке
   вывода `ps aux`, без явного предупреждения в лог. Не запускалось на
   стенде — требуется ручная проверка.

...
class XvizNode(BaseHILTest):
    """
    Управление нодой /visualization/xviz.
    Визуализация данных автономного транспортного средства.
    """

    NODE_NAME = "/visualization/xviz"
    PROCESS_NAME = "xviz_node"

    EXPECTED_ERRORS = {}  # xviz — визуализация, не влияет на control/system

    def __init__(self):
        super().__init__(node_name="xviz_node", timeout=15)
    ...
    def get_pid(self) -> int | None:
        result = self.run_docker_command(
            f"ps aux | grep {self.PROCESS_NAME} | grep -v grep | grep -v python"
        )
    ...
    def check_visualization_degradation(self) -> dict:
        """
        Проверить что система деградировала корректно —
        xviz упала, остальные ноды visualization живы.
        """
        nodes = self.get_node_list()
        viz_nodes = [n for n in nodes if 'visualization' in n.lower()]

        result = {
            "xviz_gone": self.NODE_NAME not in nodes,

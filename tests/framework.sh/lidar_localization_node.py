...
class LidarLocalizationNode(BaseHILTest):
    ...
    def check_localization_degradation(self) -> dict:
        """
        Проверить что локализация деградировала корректно —
        смежные ноды localization живы, lidar_localization отсутствует.
        """
        nodes = self.get_node_list()
        localization_nodes = [n for n in nodes if 'localization' in n.lower()]

        result = {
            "lidar_localization_gone": self.NODE_NAME not in nodes,
            "other_localization_alive": len(localization_nodes) > 0,
            "localization_nodes": localization_nodes,
            "has_auto_restart": self.has_auto_restart,
        }
        self.logger.info(f"Ноды localization после kill: {localization_nodes}")
        return result
